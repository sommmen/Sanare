...
- [ ] Complete the governed acquisition pipeline and browser-tier (Playwright) acquisition per `docs/sanare/tech-design.md`: runner integration, default Compliance (robots enforcement and bot identity), audited Stealth capability gates, robots/discovery, per-host pacing, retries/circuit breaking/challenge detection, cache and redirect policy, browsing identity, telemetry, and browser escalation.
